...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL", "https://github.com/mythex09/Faztbot.git")
...
# Terminal alias
TERM_ALIAS = os.environ.get("TERM_ALIAS", "Faztbot")
...
async def send_alive_status():
    if BOTLOG_CHATID:
        message = (
            ...
            f"┃**☛ Vᴇʀꜱɪᴏɴ:** `{python_version()}`\n"
            f"┃**☛ Oᴡɴᴇʀ:** `{ALIVE_NAME}`\n"
            f"┃**☛ Sᴛᴀᴛᴜꜱ:** `WORKING AS PHUCK`\n"
            f"┗━━━━━━━━━━━━━━━━┛\n"
        )
    ...
    try:
        ...
        async def handler(event):
            if event.message.from_id != uid:
                await event.reply("I'm [Faztbot](https://github.com/mythex09/Faztbot) modules helper...\nplease make your own bot, don't use mine 😋")
            else:
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query.startswith("@Faztbot"):
                buttons = paginate_help(0, dugmeler, "helpme")
                result = builder.article(
                    "Please Use Only With .help Command",
                    text="{}\nTotal loaded modules: {}".format(
                        "Faztbot modules helper.\n",
                        len(dugmeler),
                    ),
                    buttons=buttons,
                    link_preview=False,
                ...
                result = builder.article(
                    ...
                    buttons=[
                        [
                            custom.Button.url(
                                "GitHub Repo",
                                "https://github.com/mythex09/Faztbot"),
                            custom.Button.url(
                                "Support",
                                "https://t.me/op_squad")],
                    ],
